---
title: 生产者消费者模式
date: 2017-04-27 15:07:56.0
updated: 2017-04-27 15:15:30.0
url: /archives/279
categories: 
tags: 设计模式 | 生产者 | 消费者
---
...
<blockquote>
  生产者消费者模式应该是一个经常聊的模式了，现在经常使用的MQ是这个模式的一个常见场景了。不多赘述，直接上一个简单的代码实现。
</blockquote>

数据实体：

<pre><code>    package com.fullstacker.study.designpattern.consumer;
...
测试类中，有三个生产者和三个消费者，生产者在调用stop方法前会一直生产数据加入到BlockingQueue中，消费者不停的在queue中不停消费，在queue中的数据被消费完后，线程池结束。

运行结果如下：

<pre><code>当前线程:pool-1-thread-2, 获取了数据，id为:1, 进行装载到公共缓冲区中...
当前消费线程：pool-1-thread-6， 消费成功，消费数据为id: 1
...
当前线程:pool-1-thread-2, 获取了数据，id为:13, 进行装载到公共缓冲区中...
...
当前消费线程：pool-1-thread-4， 消费成功，消费数据为id: 13
